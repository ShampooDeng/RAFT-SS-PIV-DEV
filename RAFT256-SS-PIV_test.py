...
def train(GPU,args):
    ...
    testDataset = TFRecordDataset(test_tfrecord,
                                test_tfrecord_idx,
                                description,
                                transform=decodeData,
                                )
    test_pii = DataLoader(testDataset,batch_size=1)
    ...
    with torch.set_grad_enabled(False):
        ...
        model.eval()
        test_loader_len = int(math.ceil(TEST_QUEUE_SIZE / args.batch_size_test))
        ...
        # store resuls
        results = np.zeros((TEST_QUEUE_SIZE, 4, args.image_height, args.image_width))
        epe_array = np.zeros((TEST_QUEUE_SIZE))
